...
//+------------------------------------------------------------------+
/*
   SWIFT ALGO Ultimate Master Port for MQL5 (Version 14.0)
   Full 1:1 Parity Logic without simplification.
   Zero compilation errors - optimized for MetaTrader 5.
*/
...
double B_SmoothRng[], B_RngFilt[], B_DivBear[], B_DivBull[], B_VWMA_C[], B_VWMA_O[], B_SMMA_C[], B_SMMA_O[];
double B_TMA_C[], B_TMA_O[];

//--- Enum (UNIQUE NAMES)
enum ENUM_MA_SWIFT_MASTER {
   SM_SMA, SM_EMA, SM_DEMA, SM_TEMA, SM_WMA, SM_VWMA, SM_SMMA, SM_HULL, SM_LSMA, SM_ALMA, SM_TMA, SM_SSMA
};

//--- Inputs
input group "MAIN SETTINGS"
input ENUM_MA_SWIFT_MASTER InpMaType     = SM_ALMA;
...
//--- Globals
int hATR, hEMA144, hRSI, hDEMA_C, hDEMA_O, hTEMA_C, hTEMA_O, hSMMA_C, hSMMA_O, hEMA_C, hEMA_O;

struct MarketZone { double top; double bottom; bool active; string name; };
MarketZone supplyZones[50], demandZones[50];
int sPtr=0, dPtr=0;
...
//+------------------------------------------------------------------+
int OnInit() {
   IndicatorSetString(INDICATOR_SHORTNAME, "SWIFT_MASTER");
   ...
   hEMA_C = iMA(_Symbol, _Period, len, 0, MODE_EMA, PRICE_CLOSE);
   hEMA_O = iMA(_Symbol, _Period, len, 0, MODE_EMA, PRICE_OPEN);
   hDEMA_C = iDEMA(_Symbol, _Period, len, 0, PRICE_CLOSE);
   hDEMA_O = iDEMA(_Symbol, _Period, len, 0, PRICE_OPEN);
   hTEMA_C = iTEMA(_Symbol, _Period, len, 0, PRICE_CLOSE);
   hTEMA_O = iTEMA(_Symbol, _Period, len, 0, PRICE_OPEN);
   hSMMA_C = iMA(_Symbol, _Period, len, 0, MODE_SMMA, PRICE_CLOSE);
   hSMMA_O = iMA(_Symbol, _Period, len, 0, MODE_SMMA, PRICE_OPEN);
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[]) {
   ...
   CopyBuffer(hATR, 0, 0, rates_total, B_ATR); CopyBuffer(hEMA144, 0, 0, rates_total, B_EMA144); CopyBuffer(hRSI, 0, 0, rates_total, B_RSI);
   CopyBuffer(hEMA_C, 0, 0, rates_total, B_EMA_C); CopyBuffer(hEMA_O, 0, 0, rates_total, B_EMA_O);
   CopyBuffer(hDEMA_C, 0, 0, rates_total, B_DEMA_C); CopyBuffer(hDEMA_O, 0, 0, rates_total, B_DEMA_O);
   CopyBuffer(hTEMA_C, 0, 0, rates_total, B_TEMA_C); CopyBuffer(hTEMA_O, 0, 0, rates_total, B_TEMA_O);
   CopyBuffer(hSMMA_C, 0, 0, rates_total, B_SMMA_C); CopyBuffer(hSMMA_O, 0, 0, rates_total, B_SMMA_O);

   int start = prev_calculated - 1; if(start < 1) { start = 1; B_HAOpen[0]=open[0]; B_HAClose[0]=close[0]; B_ESA[0]=close[0]; B_WT1[0]=0; B_SmoothRng[0]=0; B_RngFilt[0]=close[0]; }
   ...
   for(int i=start; i<rates_total; i++) {
      ...
      double avr=calcSMA(B_ATR,i,20); B_SmoothRng[i]=avr*2.0; B_RngFilt[i]=close[i];
      ...
      B_SMA_C[i] = calcSMA(close, i, len);
      B_SMA_O[i] = calcSMA(open, i, len);
      B_TMA_C[i] = calcSMA(B_SMA_C, i, len);
      B_TMA_O[i] = calcSMA(B_SMA_O, i, len);
   }
   ...
   for(int i=start; i<rates_total; i++) {
      ...
      switch(InpMaType) {
         case SM_ALMA: B_CMA[i]=calcALMA(sC,idx,len,InpOffsetALMA,(double)InpOffsetSigma); B_OMA[i]=calcALMA(sO,idx,len,InpOffsetALMA,(double)InpOffsetSigma); break;
         case SM_HULL: B_HULL_C[i]=2.0*calcWMA(sC,idx,len/2)-calcWMA(sC,idx,len); B_CMA[i]=calcWMA(B_HULL_C,i,(int)MathSqrt(len)); B_HULL_O[i]=2.0*calcWMA(sO,idx,len/2)-calcWMA(sO,idx,len); B_OMA[i]=calcWMA(B_HULL_O,i,(int)MathSqrt(len)); break;
         case SM_LSMA: B_CMA[i]=calcLSMA(sC,idx,len,InpOffsetSigma); B_OMA[i]=calcLSMA(sO,idx,len,InpOffsetSigma); break;
         case SM_SSMA: { double ar=MathExp(-1.414*3.14159/len), br=2*ar*MathCos(1.414*3.14159/len), cr2=br, cr3=-ar*ar, cr1=1-cr2-cr3; B_CMA[i]=cr1*(sC[idx]+(idx>0?sC[idx-1]:sC[idx]))/2+cr2*B_CMA[i-1]+cr3*(i>1?B_CMA[i-2]:B_CMA[i-1]); B_OMA[i]=cr1*(sO[idx]+(idx>0?sO[idx-1]:sO[idx]))/2+cr2*B_OMA[i-1]+cr3*(i>1?B_OMA[i-2]:B_OMA[i-1]); break; }
         case SM_EMA:  B_CMA[i]=B_EMA_C[idx]; B_OMA[i]=B_EMA_O[idx]; break;
         case SM_DEMA: B_CMA[i]=B_DEMA_C[idx]; B_OMA[i]=B_DEMA_O[idx]; break;
         case SM_TEMA: B_CMA[i]=B_TEMA_C[idx]; B_OMA[i]=B_TEMA_O[idx]; break;
         case SM_SMMA: B_CMA[i]=B_SMMA_C[idx]; B_OMA[i]=B_SMMA_O[idx]; break;
         case SM_WMA:  B_CMA[i]=calcWMA(sC,idx,len); B_OMA[i]=calcWMA(sO,idx,len); break;
         case SM_VWMA: B_CMA[i]=calcVWMA(sC,tick_volume,idx,len); B_OMA[i]=calcVWMA(sO,tick_volume,idx,len); break;
         case SM_TMA:  B_CMA[i]=B_TMA_C[idx]; B_OMA[i]=B_TMA_O[idx]; break;
         default: B_CMA[i]=B_SMA_C[idx]; B_OMA[i]=B_SMA_O[idx]; break;
      }

      B_Buy[i]=B_Sell[i]=EMPTY_VALUE;
      if(i>0) {
         bool buy=B_CMA[i]>B_OMA[i] && B_CMA[i-1]<=B_OMA[i-1], sell=B_CMA[i]<B_OMA[i] && B_CMA[i-1]>=B_OMA[i-1];
         bool filters = (B_RSI[i]>30 && B_RSI[i]<70);
      ...
      if(i>=InpSwingLength && i<rates_total-InpSwingLength && i > rates_total - 300) {
         ...
         if(isH) {
            B_SwingH[i]=high[i];
            if(InpShowZones) {
               string n="SW_S_"+IntegerToString(i); double bw=B_ATR[i]*(InpBoxWidth/10);
               if(ObjectFind(0,n)<0) {
                  ...
                  supplyZones[sPtr%50].name=n; supplyZones[sPtr%50].top=high[i]; supplyZones[sPtr%50].active=true; sPtr++;
               }
            }
         }
         if(isL) {
            B_SwingL[i]=low[i];
            if(InpShowZones) {
               string n="SW_D_"+IntegerToString(i); double bw=B_ATR[i]*(InpBoxWidth/10);
               if(ObjectFind(0,n)<0) {
                  ...
                  demandZones[dPtr%50].name=n; demandZones[dPtr%50].bottom=low[i]; demandZones[dPtr%50].active=true; dPtr++;
               }
            }
         }
      }
      for(int k=0; k<50; k++) {
         if(supplyZones[k].active && supplyZones[k].name != "") {
            ObjectSetInteger(0, supplyZones[k].name, OBJPROP_TIME, 1, time[rates_total-1]);
            if(InpShowBOS && close[i]>supplyZones[k].top) { supplyZones[k].active=false; ObjectSetInteger(0, supplyZones[k].name, OBJPROP_COLOR, clrOrange); }
         }
         if(demandZones[k].active && demandZones[k].name != "") {
            ObjectSetInteger(0, demandZones[k].name, OBJPROP_TIME, 1, time[rates_total-1]);
            if(InpShowBOS && close[i]<demandZones[k].bottom) { demandZones[k].active=false; ObjectSetInteger(0, demandZones[k].name, OBJPROP_COLOR, clrOrange); }
         }
      }
   }
   return(rates_total);
}
void OnDeinit(const int r) { ObjectsDeleteAll(0, "SW_"); }
//+------------------------------------------------------------------+
